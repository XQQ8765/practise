 ...
 */
public class PollerAdviceAdapter extends AdviceAdapter {
    private boolean isRetransformAgent;
    private int agentId;

    protected PollerAdviceAdapter(MethodVisitor mv, int access, String name, String desc) {
        super(ASM_API, mv, access, name, desc);
        this.isRetransformAgent = false;
    }

    protected PollerAdviceAdapter(MethodVisitor mv, int access, String name, String desc, int agentId) {
        super(ASM_API, mv, access, name, desc);
    ...
    protected void onMethodExit(int opcode) {
        ...
        */
        ...
        if (isRetransformAgent) {
            mv.visitLdcInsn("poll urls" + agentId + ":");
        } else {
            mv.visitLdcInsn("poll urls:");
        }
